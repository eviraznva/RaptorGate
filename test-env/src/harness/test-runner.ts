...
export class DetachedCommand {
	...
	constructor(
		host: KnownHost,
		command: string,
		handle: import("../ssh-helper").SshProcessHandle,
	) {
		this.host = host;
...
class RequestBuilder<M extends RpcMethodName> {
	...
	private async invokeRpc(): Promise<any> {
		...
		return new Promise((resolve, reject) => {
			client[this.rpc.charAt(0).toLowerCase() + this.rpc.slice(1)](
				this.body,
...
class CommandBuilder {
	...
	async run(): Promise<void> {
		...
		try {
			...
			if (this.eventPatterns) {
				const result = await eventCollector.waitForSubsequence(
					this.eventPatterns,
				);
				if (!result.matched) {
					throw new Error(
	...
	private async invokeCommand(): Promise<{
		stdout: string;
		stderr: string;
		exitCode: number;
	}> {
