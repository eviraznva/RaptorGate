...
export interface UpdateLdapProfileCommand extends LdapProfileInputDto {
  accessToken: string;
  id: string;
}

@Injectable()
export class UpdateLdapProfileUseCase {
  constructor(
  ...
  async execute(command: UpdateLdapProfileCommand) {
    ...
    return this.identityConfigRepository.mutate(async (config) => {
      ...
      if (!existing) throw new IdentityProfileNotFoundException('ldap', command.id);
      ...
      const next = this.mutationService.replaceLdapProfile(config, profile);
