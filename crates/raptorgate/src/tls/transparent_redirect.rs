...
impl TransparentRedirect {
    pub fn new(listen_addr: SocketAddr, capture_interfaces: Vec<String>) -> Result<Self> {
    ...
    chain prerouting {{
        ...
        iifname {{ {interfaces} }} tcp dport 443 redirect to :{listen_port}
    }}
}}
",
...
mod tests {
    ...
    fn new_rejects_empty_interfaces() {
        let result = TransparentRedirect::new("127.0.0.1:8443".parse().unwrap(), vec![]);
