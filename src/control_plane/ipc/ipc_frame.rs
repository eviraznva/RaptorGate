...
impl IpcFrame {
    ...
    pub fn new(
        ...
        Ok(Self {
            ...
            kind,
            flags,
            opcode: VarInt::new(opcode),
            status: VarInt::new(status),
    ...
    pub fn encode(&self) -> Bytes {
        ...
        Self::put_varint(&mut bytes, self.opcode);
        Self::put_varint(&mut bytes, self.status);
    ...
    pub async fn read_from<R>(reader: &mut R) -> Result<Self, IpcFrameError>
    where
        R: AsyncRead + Unpin,
    {
        ...
        let opcode = Self::read_varint_from(reader, "opcode").await?;
        let status = Self::read_varint_from(reader, "status").await?;
    ...
    pub fn decode(buf: &[u8]) -> Result<(Self, usize), IpcFrameError> {
        ...
        let opcode = Self::decode_varint(&mut cursor, "opcode")?;
        let status = Self::decode_varint(&mut cursor, "status")?;
    ...
    async fn read_payload_from<R>(
        reader: &mut R,
        payload_len: usize,
    ) -> Result<Bytes, IpcFrameError>
    where
        R: AsyncRead + Unpin,
    {
    ...
    async fn read_varint_from<R>(
        reader: &mut R,
        field: &'static str,
    ) -> Result<VarInt, IpcFrameError>
    where
        R: AsyncRead + Unpin,
    {
        let mut buf = [0u8; VarInt::MAX_LEN];
    ...
    async fn read_varlong_from<R>(
        reader: &mut R,
        field: &'static str,
    ) -> Result<VarLong, IpcFrameError>
    where
        R: AsyncRead + Unpin,
    {
        let mut buf = [0u8; VarLong::MAX_LEN];
    ...
    async fn read_byte_from<R>(reader: &mut R, field: &'static str) -> Result<u8, IpcFrameError>
    where
        R: AsyncRead + Unpin,
    {
