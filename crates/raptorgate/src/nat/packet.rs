...
pub(crate) struct ParsedPacket {
    pub flow: FlowTuple,
    pub ip_headers: IpHeaders,
    pub transport: TransportHeaderOwned,
...
pub(crate) fn parse_flow_tuple_from_ethernet(data: &[u8]) -> Option<FlowTuple> {
    parse_packet(data).map(|p| p.flow)
}
...
pub(crate) fn apply_translation(data: &mut [u8], original: &FlowTuple, translated: &FlowTuple) -> bool {
...
fn rewrite_network_addresses(ip_headers: &mut IpHeaders, original: &FlowTuple, translated: &FlowTuple) -> bool {
    match ip_headers {
        IpHeaders::Ipv4(ipv4, _) => {
            if original.src_ip != translated.src_ip {
                let IpAddr::V4(src) = translated.src_ip else { return false; };
                ipv4.source = src.octets();
            }
            
            if original.dst_ip != translated.dst_ip {
                let IpAddr::V4(dst) = translated.dst_ip else { return false; };
        ...
        IpHeaders::Ipv6(ipv6, _) => {
            if original.src_ip != translated.src_ip {
                let IpAddr::V6(src) = translated.src_ip else { return false; };
                ipv6.source = src.octets();
            }
            
            if original.dst_ip != translated.dst_ip {
                let IpAddr::V6(dst) = translated.dst_ip else { return false; };
...
fn rewrite_transport_ports_tcp(header: &mut TcpHeader, original: &FlowTuple, translated: &FlowTuple) {
...
fn rewrite_transport_ports_udp(header: &mut UdpHeader, original: &FlowTuple, translated: &FlowTuple) {
    if original.src_port != translated.src_port {
        header.source_port = translated.src_port;
    }
...
fn write_tcp_header(data: &mut [u8], transport_offset: usize, tcp: &TcpHeader) -> bool {
    ...
    tcp.write(&mut Cursor::new(&mut data[transport_offset..transport_offset + header_len])).is_ok()
}
...
fn write_udp_header(data: &mut [u8], transport_offset: usize, udp: &UdpHeader) -> bool {
    ...
    udp.write(&mut Cursor::new(&mut data[transport_offset..transport_offset + UdpHeader::LEN])).is_ok()
}

#[allow(dead_code)]
pub(crate) fn same_ip_family(lhs: IpAddr, rhs: IpAddr) -> bool {
    matches!((lhs, rhs), (IpAddr::V4(_), IpAddr::V4(_)) | (IpAddr::V6(_), IpAddr::V6(_)))
}
