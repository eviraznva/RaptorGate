...
export function initializeHarness(): void {
	...
	server.bindAsync(
		...
		(err, port) => {
			...
			console.log(
				`[setup] gRPC event server listening on 0.0.0.0:${port}`,
			);
			startSshTunnel(server, {
				onReady: ({
					...
					setSnapshotClient(
						snapshotClient as FirewallConfigSnapshotServiceClient,
					);
					console.log(
						"[setup] System ready — event server + query/snapshot clients connected",
					);
					readyResolve();
				},
			});
...
export async function waitForReady(): Promise<void> {
	if (!readyPromise) {
		throw new Error(
			"Harness not initialized — call initializeHarness() first",
		);
	}
	return readyPromise;
}
