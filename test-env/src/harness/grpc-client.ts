...
export function setSnapshotClient(
	client: FirewallConfigSnapshotServiceClient,
): void {
...
export function getClient(): FirewallQueryServiceClient {
	if (!_client) {
		throw new Error(
			"gRPC query client not initialized — has setup.ts preload run?",
		);
	}
	return _client;
}
...
export function getSnapshotClient(): FirewallConfigSnapshotServiceClient {
	if (!_snapshotClient) {
		throw new Error(
			"gRPC snapshot client not initialized — has setup.ts preload run?",
		);
	}
