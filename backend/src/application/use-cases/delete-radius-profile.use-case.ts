...
export interface DeleteRadiusProfileCommand {
  accessToken: string;
  id: string;
}

@Injectable()
export class DeleteRadiusProfileUseCase {
  constructor(
  ...
  async execute(command: DeleteRadiusProfileCommand) {
    ...
    return this.identityConfigRepository.mutate(async (config) => {
      const next = this.mutationService.deleteRadiusProfile(config, command.id);
