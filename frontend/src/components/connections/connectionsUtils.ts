...
export function formatInterfacePath(session: TcpTrackedSession) {
  const { originalIngress, originalEgress, replyIngress, replyEgress } = session.interfaces;

  return `${originalIngress || "?"} -> ${originalEgress || "?"} / ${replyIngress || "?"} -> ${replyEgress || "?"}`;
}

export function getTotalBytes(session: TcpTrackedSession) {
  return session.bytesOriginal + session.bytesReply;
}

export function getTotalPackets(session: TcpTrackedSession) {
  return session.packetsOriginal + session.packetsReply;
}
...
export function getTotalSessionBytes(sessions: TcpTrackedSession[]) {
  return sessions.reduce((total, session) => total + getTotalBytes(session), 0);
}

export function getTotalSessionPackets(sessions: TcpTrackedSession[]) {
  return sessions.reduce((total, session) => total + getTotalPackets(session), 0);
}
