...
mod tests {
    ...
    fn test_dns_response_updates_cached_query() {
        ...
        match &r2 {
            InspectResult::Done(ctx) => {
                ...
                assert_eq!(ctx.dns_answer_types, vec![dns::DnsRecordType::A]);
    ...
    fn test_dns_response_txt_tunneling() {
        ...
        let query = dns_query_payload("exfil.suspect.com", 16);
        ...
        let resp = dns_response_payload("exfil.suspect.com", 16, &[(16, &big_txt)]);
        ...
        match &r {
            InspectResult::Done(ctx) => {
                assert_eq!(ctx.dns_query_name.as_deref(), Some("exfil.suspect.com"));
                assert_eq!(ctx.dns_query_type, Some(dns::DnsRecordType::Txt));
                assert_eq!(ctx.dns_answer_count, 1);
                assert_eq!(ctx.dns_answer_types, vec![dns::DnsRecordType::Txt]);
    ...
    fn test_dns_multiple_queries_same_flow() {
        ...
        match &r2 {
            InspectResult::Done(ctx) => {
                ...
                assert_eq!(ctx.dns_query_type, Some(dns::DnsRecordType::Aaaa));
                assert_eq!(ctx.dns_answer_count, 0);
        ...
        match &r3 {
            InspectResult::Done(ctx) => {
                assert_eq!(ctx.dns_query_name.as_deref(), Some("second.example.com"));
                assert_eq!(ctx.dns_answer_count, 1);
                assert_eq!(ctx.dns_answer_types, vec![dns::DnsRecordType::Aaaa]);
            }
