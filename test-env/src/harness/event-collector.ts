...
export class EventCollector {
	...
	push(event: any): void {
		...
		if (this.printingEnabled) {
			...
			console.log(
				`[DEBUG EVENT] ${kind}:`,
				JSON.stringify(event.kind?.[kind] ?? event, null, 2),
			);
		}
	...
	async waitForSubsequence(patterns: EventMatcher[]): Promise<WaitForResult> {
	...
	): WaitForResult & { terminated: boolean } {
		let patternIdx = 0;
	...
	private isAfterFence(event: BufferedEvent): boolean {
		...
		const eventMs =
			seconds * 1000 + (event.emittedAt.nanos ?? 0) / 1_000_000;
	...
	private extractPayload(
		...
		return (
			(event.kind[kind] as Record<string, unknown> | undefined) ?? null
		);
	}
	...
	private shallowMatch(
		...
		for (const [key, value] of Object.entries(expected)) {
			...
			if (
				...
				if (!this.shallowMatch(actual[key] as any, value as any))
					return false;
