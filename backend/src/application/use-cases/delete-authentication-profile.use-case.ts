...
export interface DeleteAuthenticationProfileCommand {
  accessToken: string;
  id: string;
}

@Injectable()
export class DeleteAuthenticationProfileUseCase {
  constructor(
  ...
  async execute(command: DeleteAuthenticationProfileCommand) {
    ...
    return this.identityConfigRepository.mutate(async (config) => {
      const next = this.mutationService.deleteAuthenticationProfile(
        config,
        command.id,
      );
