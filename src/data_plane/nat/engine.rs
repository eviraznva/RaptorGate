...
impl NatEngine {
    ...
    pub fn process_prerouting(&mut self, data: &mut [u8], in_interface: &str, in_zone: Option<&str>)
        -> NatOutcome
    {
        self.process_stage(data, NatStage::Prerouting, Some(in_interface), None, in_zone, None)
    }

    pub fn process_postrouting(&mut self, data: &mut [u8], out_interface: &str, out_zone: Option<&str>)
        -> NatOutcome
    {
        self.process_stage(data, NatStage::Postrouting, None, Some(out_interface), None, out_zone)
    }
    ...
    ) -> NatOutcome
    {
        ...
        if let Some((binding_id, direction)) = self.bindings.lookup(&flow_tuple) {
            ...
            return NatOutcome::AppliedExisting { binding_id, direction };
        }

        let Some(rule) = self.find_matching_rule(
            stage, &flow_tuple, in_interface, out_interface, in_zone, out_zone,
        ) else {
    ...
    fn parse_flow_tuple(packet: &SlicedPacket) -> Option<FlowTuple> {
        ...
        let (proto, src_port, dst_port) = match &packet.transport {
            Some(TransportSlice::Tcp(tcp))  => (L4Proto::Tcp,  tcp.source_port(), tcp.destination_port()),
            Some(TransportSlice::Udp(udp))  => (L4Proto::Udp,  udp.source_port(), udp.destination_port()),
            Some(TransportSlice::Icmpv4(_)) => (L4Proto::Icmp, 0u16,              0u16),
            _ => return None,
        };

        Some(FlowTuple { src_ip, dst_ip, src_port, dst_port, proto })
    }
    
    fn apply_rewrite(data: &mut [u8], original: &FlowTuple, translated: &FlowTuple) {
        const ETH: usize = 14;
        
        let Ok((mut ipv4, _)) = Ipv4Header::from_slice(&data[ETH..]) else { return };
        ...
        match ipv4.protocol {
            etherparse::IpNumber::TCP => {
                let Ok((mut tcp, _)) = TcpHeader::from_slice(&data[tp..]) else { return };
                let tcp_hdr_len = tcp.header_len();

                if original.src_port != translated.src_port { tcp.source_port      = translated.src_port; }
                if original.dst_port != translated.dst_port { tcp.destination_port = translated.dst_port; }
            ...
            etherparse::IpNumber::UDP => {
                let Ok((mut udp, _)) = UdpHeader::from_slice(&data[tp..]) else { return };
                let udp_hdr_len = UdpHeader::LEN;

                if original.src_port != translated.src_port { udp.source_port      = translated.src_port; }
                if original.dst_port != translated.dst_port { udp.destination_port = translated.dst_port; }
    ...
    fn find_matching_rule(
        ...
        nat_rules.rules().iter().find(|rule| {
            if !stage_actions.contains(&rule.action()) { return false; }

            if let Some(ri) = rule.in_interface()  { if in_interface  != Some(ri) { return false; } }
            if let Some(ri) = rule.out_interface() { if out_interface != Some(ri) { return false; } }
            if let Some(rz) = rule.in_zone()       { if in_zone       != Some(rz) { return false; } }
            if let Some(rz) = rule.out_zone()      { if out_zone      != Some(rz) { return false; } }

            if let Some(cidr) = rule.src_cidr() { if !cidr.contains(&flow.src_ip) { return false; } }
            // For DNAT, dst_cidr is the translation target (not a match condition)
            if rule.action() != NatAction::Dnat {
                if let Some(cidr) = rule.dst_cidr() { if !cidr.contains(&flow.dst_ip) { return false; } }
            }
            ...
            if let Some(rule_proto) = rule.protocol() {
                ...
                if rule_proto != flow_proto { return false; }
            }

            // For DNAT, src_port is the translation target port (not a match condition)
            if rule.action() != NatAction::Dnat {
                if let Some(port) = rule.src_port() { if flow.src_port != port { return false; } }
            }
            if let Some(port) = rule.dst_port() { if flow.dst_port != port { return false; } }
