...
const SnapshotServiceClient =
	proto.raptorgate.services.FirewallConfigSnapshotService;
...
async function isVmRunning(): Promise<boolean> {
	try {
		const { stdout, exitCode } = await run("vagrant", ["status", VM_NAME], {
			cwd: VAGRANT_DIR,
		});
...
function createEventTunnelLoop(
	...
	async function loop(): Promise<void> {
		while (true) {
			...
			eventTunnelProc = startSshTunnelProcess(
				"-R",
				String(TUNNEL_LOCAL_PORT),
				TUNNEL_REMOTE_SOCKET,
			);
			...
			await new Promise<void>((resolve) => {
				...
				eventTunnelProc.on("error", (err) => {
					console.error(
						`[event-tunnel] Tunnel process error: ${err.message}`,
					);
				});

				eventTunnelProc.on("close", (code, signal) => {
					console.log(
						`[event-tunnel] Tunnel closed (code=${code}, signal=${signal}). Reconnecting ...`,
					);
					resolve();
				});
	...
	async function loop(): Promise<void> {
		while (true) {
			...
			try {
				...
				console.error(
					"[query-tunnel] Failed to create gRPC client:",
					err,
				);
			...
			await new Promise<void>((resolve) => {
				...
				queryTunnelProc.on("error", (err) => {
					console.error(
						`[query-tunnel] Tunnel process error: ${err.message}`,
					);
				});

				queryTunnelProc.on("close", (code, signal) => {
					console.log(
						`[query-tunnel] Tunnel closed (code=${code}, signal=${signal}). Reconnecting ...`,
					);
					resolve();
				});
				...
				onReady();
				console.log(
					"[query-tunnel] Tunnel established, gRPC client connected",
				);
			});
